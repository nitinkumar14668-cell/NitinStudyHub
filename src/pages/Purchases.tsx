...
export default function Purchases({ user }: PurchasesProps) {
  ...
  if (!user) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        <div className="bg-white p-12 rounded-[3.5rem] border border-gray-100 shadow-xl inline-block max-w-md">
          ...
          <p className="text-gray-500 font-medium mb-8">You need to be logged in to view your purchases and downloads.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      <div className="mb-12">
        <h1 className="text-5xl font-black text-gray-900 tracking-tight mb-4">My Purchases</h1>
        <p className="text-gray-500 font-medium text-lg">Tracks your orders and download your verified PDFs here.</p>
      </div>

      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1, 2].map(i => (
            <div key={i} className="bg-gray-100 h-40 rounded-[2.5rem] animate-pulse" />
          ))}
        </div>
      ) : purchases.length === 0 ? (
        <div className="bg-white p-20 rounded-[3.5rem] text-center border border-gray-100 shadow-xl">
          <div className="bg-blue-50 w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-6">
            <ShoppingBag className="w-10 h-10 text-blue-600" />
          </div>
          <h2 className="text-3xl font-black text-gray-900 mb-4">No purchases yet</h2>
          <p className="text-gray-500 font-medium mb-8 max-w-md mx-auto">Explore our store and find the best notes for your preparation.</p>
          <Link to="/" className="inline-flex items-center gap-3 bg-gray-900 text-white px-8 py-4 rounded-2xl font-black hover:bg-gray-800 transition-all shadow-xl shadow-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {purchases.map((p) => (
            <motion.div
              ...
              className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-xl relative overflow-hidden"
            >
              ...
              <div className="space-y-4 mb-8">
                <div className="flex justify-between items-center text-sm font-medium p-4 bg-gray-50 rounded-2xl">
                  ...
                  <span className="text-gray-900 font-bold">₹{p.amount}</span>
                </div>
                {p.status === TransactionStatus.APPROVED ? (
